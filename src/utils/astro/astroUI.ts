...
> = {
  ...
  Pluto: { name: '冥王星', color: mixHexColors(eColors.Water, '#000000', 0.4), symbol: '♇' }, // 深紫（转化/深层力量）
  // Sun: { name: '太阳', n: '日', color: eColors.Fire, symbol: '☉' }, // 红色（太阳）
...
export const DignityMap: Record<DignityStatus, { color: string; text: string; score: string }> = {
  ...
  Exaltation: { text: '旺', color: '#67c23a', score: '+4' }, // 较强（蓝色）
  Detriment: { text: '失势', color: '#f56c6c', score: '-5' }, // 极弱（红色）
  Fall: { text: '落陷', color: '#f56c6c', score: '-4' }, //  弱化（橙色）
  Peregrine: { text: '', color: '#909399', score: '0' }, // 中性（灰色）
  DetrimentFall: { text: '失势 落陷', color: '#f56c6c', score: '-9' },
};
